...
class Message(object):
    ...
    def apply_pad(self, pad):
        ...
        '''
        original = list(self.get_text())
        ciphertext = []
        if len(pad) == len(original):
            for i in range(len(original)):
                ciphertext.append(self.shift_char(original[i], pad[i]))
        else:
            raise ValueError
...
class PlaintextMessage(Message):
    def __init__(self, input_text, pad=None):
        ...
        self.pad = pad.copy() if pad != None else self.generate_pad().copy()
    ...
    def generate_pad(self):
        ...
        '''
        return [random.randint(0, 110) for i in range(len(self.text))]
