...
def decrypt_message_try_pads(ciphertext, pads):
    ...
    max_score = -1
    messages = [ciphertext.decrypt_message(pad) for pad in pads]
    best_message = None
    for message in messages:
        score = sum(is_word(words, word) for word in message.get_text().split())
        if score >= max_score:
            max_score = score
...
def decode_story():
    ...
    pads = get_story_pads()
    return decrypt_message_try_pads(story_string, pads).get_text()
