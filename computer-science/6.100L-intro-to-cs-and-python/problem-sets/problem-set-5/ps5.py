...
def filter(pixels_list, color):
    ...
    for pixel in pixels_list:
        filtered_pixel = tuple(int(i) for i in matrix_multiply(transformation_matrix, pixel))
        result.append(filtered_pixel)
    return result
...
def reveal_bw_image(filename):
    ...
    return pix_to_img(hidden_pixels, (width, height), 'L')
...
def reveal_color_image(filename):
    ...
    hidden_pixels = []
    for pixel in pixels:
        hidden_pixels.append(tuple(int(x*255/7) for x in extract_end_bits(3, pixel)))
    return pix_to_img(hidden_pixels, (width, height), 'RGB')
...
def main():
    ...
    # im2.show()
    # im2.save('red_filtered_image_15.png')
    ...
    # kerb = 'github.com/johnny-m-young'
    # draw_kerb('red_filtered_image_15.png', kerb)
    # draw_kerb('unhidden1.bmp', kerb)
    # draw_kerb('unhidden2.bmp', kerb)
